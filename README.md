...
**Export and import bundles.** A project can be exported as a single portable `.zip` bundle using the "↓ Export bundle" button in the project overview header. The bundle captures the complete project folder — source documents, indexed evidence, generated drafts, approved sections, and the full audit trail — so the recipient gets an exact replica. To import a bundle, click "Import bundle" in the project list header, select the `.zip` file, and the project is recreated in the current AgCensus folder. If a project with the same name already exists, a confirmation prompt asks whether to overwrite it before proceeding. This workflow is the recommended way to share a project between machines or transfer it to a colleague.
...
**Multi-provider model selection.** The application supports fourteen models across six providers, organised into three tiers. Budget tier (Tier 1) includes DeepSeek V4 Flash, Gemini 2.0 Flash, GPT-4o mini, Claude Haiku 4.5, and Azure GPT-4o mini. Mid-range tier (Tier 2) includes DeepSeek V4 Pro, Kimi K2.6, Kimi K2.6 Thinking, Gemini 2.5 Flash, and Claude Sonnet 4.6. Premium tier (Tier 3) includes GPT-4o, Gemini 2.5 Pro, Claude Opus 4.8, and Azure GPT-4o. Azure OpenAI models are available for FAO Microsoft 365 enterprise deployments and require an endpoint URL and deployment name in addition to an API key. The model selector in the MR and TMR review screens shows a cost estimate before generation begins. Before a full TMR run with a mid-range or premium model a cost confirmation dialog is shown; users are encouraged to use a Budget model for the initial full run and then regenerate weak sections or sub-tables individually with a Premium model.

**Managing cost.** Budget models (DeepSeek V4 Flash, Gemini 2.0 Flash) cost a few cents for a full 23-sub-table TMR run. Premium models (Claude Sonnet 4.6, Claude Opus 4.8, GPT-4o) can cost several dollars for a full run because sub-tables with many rows (livestock by type: 22 rows; crops: 15+ rows) multiply token usage by their row count. The recommended workflow is: generate all sub-tables with a Budget model, review the output, then regenerate only the weak sub-tables individually with a Premium model using the per-sub-table "Generate sub-table" button. Each MR section can similarly be regenerated individually. This reduces full-run Premium costs by 10–50×.
