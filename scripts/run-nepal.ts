/**
 * scripts/run-nepal.ts — Session 10 end-to-end pipeline run for Nepal.
 *
 * Creates a fresh Nepal project in a temp directory, ingests the Nepal 2021/2022
 * main census report, generates all 15 MR sections and 23 TMR sub-tables using
 * deepseek-v4-flash, then writes a summary to docs/nepal-run-summary.json.
 *
 * Run with:
 *   node "C:\Users\Dramane\Desktop\PIPELINE\node_modules\vitest\vitest.mjs" ^
 *     run --root "C:\Users\Dramane\Desktop\PIPELINE" ^
 *     scripts/run-nepal.ts --reporter verbose
 *
...
/**
 * Walk up from scripts/ to find the real project root — works in both the
 * normal layout (scripts/ → project-root) and the worktree layout
 * (scripts/ → worktree → worktrees/ → .claude/ → project-root).
 */
...
/**
 * Load .env from the project root and set any missing env vars.
...
// Load .env before the API-key check runs at module level
await loadEnvFile();
...
if (!apiKeyPresent) {
  console.warn(
    "\n[run-nepal] SKIP — DEEPSEEK_API_KEY is not set.\n" +
...
try {
  ...
  console.warn(
    "\n[run-nepal] SKIP — Nepal census PDF not found at:\n" +
      `  ${PDF_PATH}\n`,
  );
}
...
// ---------------------------------------------------------------------------
// Internal types for the run summary
// ---------------------------------------------------------------------------
...
test(
  ...
  async (ctx) => {
    ...
    const projectDir = await mkdtemp(
      path.join(os.tmpdir(), "agcensus-nepal-"),
    );
    console.log(`\n[Nepal] Project dir: ${projectDir}`);
    await createProject(projectDir, FIELDS);

    // ── 2. Ingest PDF ──────────────────────────────────────────────────────
    console.log(`[Nepal] Ingesting: ${PDF_PATH}`);
    ...
    console.log(
      `[Nepal] Indexed ${pagesIndexed} pages, ${tablesIndexed} tables in ${ingestTimeMs} ms`,
    );
    ...
    for (let s = 1; s <= 15; s++) {
      console.log(`[Nepal] MR section ${s}/15 ...`);
      const wallStart = Date.now();
      await generateSection(projectDir, s, MODEL);
      mrResults[`section_${s}`] = {
        status: "ok",       // refined after reading _claims.json
        claims_count: 0,
    ...
    for (let t = 1; t <= 23; t++) {
      console.log(`[Nepal] TMR sub-table ${t}/23 ...`);
      const wallStart = Date.now();
      await generateSubTable(projectDir, t, MODEL);
      tmrResults[`sub_table_${t}`] = {
        status: "ok",       // refined after reading _cells.json
        cells_populated: 0,
        cells_missing: 0,
    ...
    for (let s = 1; s <= 15; s++) {
      ...
      // If the section key is absent → parse failed (generateSection wrote
      // the raw text to current.md but skipped _claims.json).
      // If present but empty → all claims lacked source citations.
    ...
    const mrStatusCounts = {
      ...
      empty: Object.values(mrResults).filter((r) => r.status === "empty").length,
    ...
    const summaryPath = path.join(docsDir, "nepal-run-summary.json");
    ...
    console.log(`\n${line}`);
    console.log("NEPAL E2E RUN COMPLETE");
    console.log(line);
    console.log(`Project dir  : ${projectDir}`);
    console.log(`PDF          : ${pagesIndexed} pages, ${tablesIndexed} tables indexed`);
